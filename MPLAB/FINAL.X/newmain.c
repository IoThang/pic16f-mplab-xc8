...
#define TAYPHAI         PORTAbits.RA3
#define TAYTRAI         PORTAbits.RA5
...
void main(void) {
    ...
    while(1)
    {
        ...
        //Lay gia tri tin cam bien sieu am
        u16distance = get_val();
        unsigned char temp;
        temp = u16distance / 58;
        //Do line
        ...
        if((1 == CenterSensor) && (1 == LeftSensor) && (1 == RightSensor))
            car(8);
        //Kiem tra gia tri cam bien sieu am. Neu value nho hon < 5 thi dung chay
        if(temp < 5)
        {
            __delay_ms(100);
            if(temp < 5)
