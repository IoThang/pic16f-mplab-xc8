...
{    
    ...
    //TRISB =  0x00;  // TRIS = 0 means output mode
    TRISD  =  0x00; 
    ANSEL  = 0x00; // ANSELX = 0 deselects analog mode 
    ANSELH = 0x00;
    
    // power up the LCD
    //LCD_PWR = 1; This line is no use
...
{
    // mask off the nibble to be transmitted (applied for both higher and lower bits)
    ch = (ch & 0xF0);   // ch: XXXXXXXX -> ch: XXXX0000

    // clear the LCD_BUS
    LCD_REG = (LCD_REG & 0x0F); // No change to the RS, RW and E pin
    LCD_DB4 = 0;
    LCD_DB5 = 0;
    LCD_DB6 = 0;
    LCD_DB7 = 0;
    
    // move the nibble onto LCD_BUS
    // Latch to DB7
    if(((ch & 0x80)>>7)==1){
        LCD_DB7 = 1;
    } else{
    ...
    // You may also use this simplest way:
    // TRISB = ch;
